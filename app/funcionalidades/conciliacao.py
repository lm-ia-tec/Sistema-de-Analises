...
# =========================================================
# FUNÇÕES DE LÓGICA DE NEGÓCIO (CONCILIAÇÃO)
# =========================================================
...
def limpar_df_prefeitura(df):
    ...
    df = df.copy()
    if 'CPF/CNPJ Prestador' in df.columns:
        df['CPF/CNPJ Prestador'] = df['CPF/CNPJ Prestador'].apply(utils.format_cnpj)
...
def limpar_df_financeiro(df):
    ...
    if 'Crédito' in df.columns:
        ...
        if pd.api.types.is_numeric_dtype(df['Crédito']):
             ...
             df['Crédito'] = utils.parse_moeda_brasil_robusto(df['Crédito'])
    else:
        df['Crédito'] = np.nan
            ...
            if nome_arquivo.endswith(('.xls', '.xlsx')):
                ...
                if 'Crédito' in df_financeiro_raw.columns:
                     if not pd.api.types.is_numeric_dtype(df_financeiro_raw['Crédito']):
                        df_financeiro_raw['Crédito'] = utils.parse_moeda_brasil_robusto(df_financeiro_raw['Crédito'])
            
            elif nome_arquivo.endswith('.csv'):
                # Para CSV (com prioridade no ;)
                df_financeiro_raw = utils.carregar_arquivo_csv(file_razao)
            else:
